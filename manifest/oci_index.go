...
func (index *OCI1Index) UpdateInstances(updates []ListUpdate) error {
	...
	for i := range updates {
		...
		if SupportedSchema2MediaType(updates[i].MediaType) != nil && updates[i].MediaType != imgspecv1.MediaTypeImageIndex {
			return errors.Errorf("update %d of %d passed to OCI1Index.UpdateInstances had an unsupported media type (was %q): %q", i+1, len(updates), index.Manifests[i].MediaType, updates[i].MediaType)
		}
